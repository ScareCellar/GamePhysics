...
*
*   Copyright (c) 2022 Cameron Troy. All Rights Reserved.
*
...
GuiPhysicsState InitGuiPhysics(void)
{
    ...
    state.EffectorSizeValue = 100.0f;
...
void GuiPhysics(GuiPhysicsState *state)
{
    ...
    if (state->PhysicsPanelActive)
    {
        ...
        GuiSliderBar(Rectangle{ state->anchor02.x + 80, state->anchor02.y + 88, 184, 16 }, "SIZE", GUI_TEXT(state->BodySizeValue), &state->BodySizeValue, 0.1, 1);
        GuiSliderBar(Rectangle{ state->anchor02.x + 80, state->anchor02.y + 136, 184, 16 }, "GRAVITY", GUI_TEXT(state->BodyGravityValue), &state->BodyGravityValue, -100, 100);
        GuiSliderBar(Rectangle{ state->anchor02.x + 80, state->anchor02.y + 160, 184, 16 }, "BOUNCE", GUI_TEXT(state->BodyRestitutionValue), &state->BodyRestitutionValue, 0, 1);
        ...
        GuiSliderBar(Rectangle{ state->anchor02.x + 80, state->anchor02.y + 272, 184, 16 }, "STIFFNESS", GUI_TEXT(state->SpringStiffnessValue), &state->SpringStiffnessValue, 0, 1);
        GuiSliderBar(Rectangle{ state->anchor02.x + 80, state->anchor02.y + 296, 184, 16 }, "DAMPING", GUI_TEXT(state->SpringDampingValue), &state->SpringDampingValue, 0, 1);
        GuiCheckBox(Rectangle{ state->anchor02.x + 104, state->anchor02.y + 344, 16, 16 }, "AUTO LENGTH", &state->SpringAutoLengthChecked);
        GuiSliderBar(Rectangle{ state->anchor02.x + 80, state->anchor02.y + 320, 184, 16 }, "LENGTH", GUI_TEXT(state->SpringLengthValue), &state->SpringLengthValue, 0, 1);
